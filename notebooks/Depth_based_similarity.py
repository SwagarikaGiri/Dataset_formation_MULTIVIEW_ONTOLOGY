...
def maximum_depth_of_ontology():
	max_depth=0
	term=None
	for key,value in Go_term_Depth_lexicon.iteritems():
		if value > max_depth:
			max_depth=value
			term=key
	return max_depth,term
...
def make_depth_based_matix(maximum_depth):
	output_file='Depth_Based_Matrix.txt'
	with open(output_file,'w') as outputcsv_file:
		spamwriter = csv.writer(outputcsv_file,delimiter=',')
		col1=[]
		col1.append("Go term ")
		...
		for i in range(0,len(considered_goterm_list)):
			...
			for j in range(0,len(considered_goterm_list)):
				...
				if LCA != "NoParent":
					...
					flag=1
					depth=0
				max_depth=float(maximum_depth)
				if flag==1:
					depth_new=depth
				else:
					depth_new=depth+1
				depth_new=float(depth_new)
				val=depth_new/max_depth
				col1.append(val)
			spamwriter.writerow(col1)



maximum_depth,go_term=maximum_depth_of_ontology()
print maximum_depth
print go_term
print Go_term_Depth_lexicon[go_term]
make_depth_based_matix(maximum_depth)